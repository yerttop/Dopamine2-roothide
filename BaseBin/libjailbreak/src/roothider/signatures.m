...
void recurse_collect_untrusted_cdhashes(const char *path, const char *callerImagePath, const char *callerExecutablePath, uint32_t *preferredArchTypes, uint32_t *preferredArchSubtypes, size_t preferredArchCount, cdhash_t **cdhashesOut, uint32_t *cdhashCountOut)
{
	@autoreleasepool {
		if (!path) return;
						...
						if (csd_superblob_calculate_best_cdhash(decodedSuperblob, cdhash, NULL) == 0) {
							if (!cdhashesContains(cdhash)) {
								if (!is_cdhash_trustcached(cdhash)) {
									...
									int ret = ensure_randomized_cdhash_for_slice(resolvedBinaryPath.fileSystemRepresentation, macho->archDescriptor.offset, cdhash);
									if(ret==0) {
										cdhashesAdd(cdhash);
									} else {
										JBLogError("ensure_randomized_cdhash_for_slice(%llx) failed: %s -> (%d)", macho->archDescriptor.offset, resolvedBinaryPath.fileSystemRepresentation, ret);
									}
								}
